...
class SeedExportBIP85GetIndexScreen(BaseTopNavScreen):
    ...
    def __post_init__(self):
        ...
        keyboard_width = self.canvas_width - (
                    GUIConstants.EDGE_PADDING + GUIConstants.COMPONENT_PADDING + right_panel_buttons_width - GUIConstants.COMPONENT_PADDING)
        text_entry_display_y = self.top_nav.height
        text_entry_display_height = 30
        ...
        self.text_entry_display = TextEntryDisplay(
            ...
            cursor_mode=TextEntryDisplay.CURSOR_MODE__BAR,
            is_centered=False,
            cur_text=''.join(self.bip85_index)
        )
    ...
    def _render(self):
        ...
        self.exit_button.render()
        self.text_entry_display.render(self.bip85_index)
        self.renderer.show_image()

    def _run(self):
        cursor_position = len(self.bip85_index)

        # Start the interactive update loop
        while True:
            input = self.hw_inputs.wait_for(
                HardwareButtonsConstants.KEYS__LEFT_RIGHT_UP_DOWN + [HardwareButtonsConstants.KEY_PRESS,
            ...
            if input == HardwareButtonsConstants.KEY3:
                ...
                self.exit_button.render()
                self.renderer.show_image()
                if len(self.bip85_index) > 0:
                    return self.bip85_index.strip()
            ...
            # Process normal input
            if input in [HardwareButtonsConstants.KEY_UP,
                         HardwareButtonsConstants.KEY_DOWN] and self.top_nav.is_selected:
                # We're navigating off the previous button
                self.top_nav.is_selected = False
                self.top_nav.render_buttons()
                ...
                if input == HardwareButtonsConstants.KEY_DOWN:
                    ...
                    input = Keyboard.ENTER_BOTTOM
            elif input in [HardwareButtonsConstants.KEY_LEFT,
                           HardwareButtonsConstants.KEY_RIGHT] and self.top_nav.is_selected:
                # ignore
                continue
            ...
            if ret_val in Keyboard.EXIT_DIRECTIONS:
                ...
                # User has locked in the current letter
                if cursor_position == len(self.bip85_index):
                    self.bip85_index += ret_val
                else:
                    self.bip85_index = self.bip85_index[:cursor_position] + ret_val + self.bip85_index[
                                                                                              cursor_position:]
                cursor_position += 1
